...
const defaultConfig = {
    ...
    mediaStrategy: definitions_1.MEDIA_STRATEGIES.UP,
    mediaSizes: definitions_1.DEFAULT_MEDIA_SIZES_UP
};
/** Helper Methods **/
...
const areMediaWidthRangesValid = ({ xl, lg, md, sm, xs }, mediaStrategy) => {
    ...
    if (mediaStrategy === definitions_1.MEDIA_STRATEGIES.UP) {
        if ((sm - xs) !== 1) {
    ...
    else if (mediaStrategy === definitions_1.MEDIA_STRATEGIES.DOWN) {
        if ((xl - lg) !== 1) {
 ...
 *
 **/
exports.processConfigObject = (config) => {
    ...
    if (Object.prototype.hasOwnProperty.call(config, "mediaStrategy")) {
        if (config.mediaStrategy === definitions_1.MEDIA_STRATEGIES.DOWN) {
            // If config overrides default media strategy, save in custom config object
            customConfig.mediaStrategy = definitions_1.MEDIA_STRATEGIES.DOWN;
            customConfig.mediaSizes = definitions_1.DEFAULT_MEDIA_SIZES_DOWN;
        }
        else if (config.mediaStrategy !== definitions_1.MEDIA_STRATEGIES.UP) {
    ...
    if (Object.prototype.hasOwnProperty.call(config, "mediaSizes")) {
        ...
        if (typeof config.mediaSizes !== "object") {
            ...
            if (isConfigMediaSizesValid) {
                ...
                const overriddenMediaSizes = Object.assign(Object.assign({}, currentMediaSizes), config.mediaSizes);
                ...
                if (areMediaWidthRangesValid(overriddenMediaSizes, currentMediaStrategy)) {
                    ...
                    customConfig = Object.assign(Object.assign({}, customConfig), { mediaSizes: overriddenMediaSizes });
                }
            }
        }
    }
